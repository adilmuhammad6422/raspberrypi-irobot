...
class Robot:
    ...
    def __call_command(self, velocity, radius):
        # Convert velocity and radius to bytes
        vel_high_byte, vel_low_byte, radius_high_byte, radius_low_byte = self.__convert_to_bytes(velocity, radius)
    ...
    def drive_straight(self, duration):
        ...
        self.__call_command(self.velocity, 32768) # Calls drive command, 32768 is the radius for driving straight
    ...
    def turn_dynamic_angle(self, angle):
        ...
        # If the angle is 0 drive straight, otherwise turn proportionally for the given angle
        if angle == 0:
            radius = 32768
        else:
            radius = 1 / (angle / 90.0)

        self.__call_command(self.velocity, int(radius))
        
        # Waits for the angle to turn
        time_to_turn = abs(angle) / 90.0  # Adjust the time calculation as necessary
    ...
    def turn_while_driving(self, radius, duration):
        print('Turning while driving...')
        self.__call_command(self.velocity, radius)
        time.sleep(duration)
